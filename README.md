...
| Metric | Result |
|---|---|
| Retrieval quality (Module 1, `dense_search`, original 28-query eval set) | **hit_rate@5 = 1.000**, **precision@1 = 0.893** — beat every hybrid/reranked variant tested. Not yet re-run against the expanded 49-query eval set (see below) — this figure still reflects the original 28. |
| End-to-end answer correctness (Module 2+3, LLM-as-judge, 49-query eval set) | **39 / 49 correct** — 38/45 standard (fact-lookup + narrative) queries correct, plus 1/4 cross-company comparative queries correctly declined (see "Known Limitations" below for why that comparative number is low) |
| Red-team audit catch rate on observed fabrications (Module 3) | **1 / 1 (100%, small sample)** |
| Calibrated confidence coverage (Module 4, split-conformal, stratified 25/24 calibration/test split, 80% target) | **83.3%** (15/18 high-confidence test cases actually correct, threshold 0.9820) |
...
| **Module 4** | Calibrated confidence via split-conformal prediction over a continuous non-conformity score (audit status + retrieval similarity + spread + length), replacing a degenerate discrete-score first attempt | [README_module4.md](README_module4.md) | **83.3% empirical coverage** against an 80% target on a stratified 24-query test set (15/18 high-confidence cases correct, threshold 0.9820) |
...
- **A defense-in-depth gap: `detect_company()`'s text-based fallback has no safeguard against cross-company comparative queries on its own.** `app.py`'s UI requires an explicit company selection before running a query, which sidesteps this in normal interactive use. But when the underlying pipeline is called directly — as the eval harness does, bypassing the UI entirely — `answer_with_self_correction` falls back to `detect_company()`'s query-text inference, which has no concept of "this question asks about multiple companies at once." Testing this directly: **3 of 4 cross-company comparative queries** (e.g., "Which of the five companies has the strongest competitive position?") **produced a confidently wrong single-company answer instead of declining or flagging the question as out of scope.** `detect_company()` either matches the first company name it happens to find in the query text and silently filters to just that company, or finds no match and searches all 5 companies' chunks unfiltered — and generation then answers as if it had comprehensive cross-company evidence either way. The UI layer is a real mitigation for interactive use, but it is not a substitute for a safeguard at the pipeline level, and this is documented here as a known architectural limitation rather than silently relying on the UI alone. Full writeup: [README_module2.md](README_module2.md)'s "Cross-Company Comparative Queries" section and the 49-query eval set's `graceful_decline`-tagged entries (`eval/eval_set.json`).
- **Small sample sizes throughout should temper every number above.** The eval set now has 49 queries (up from 28) with a stratified 25/24 calibration/test split (up from an accidentally lopsided 14/35 before the split logic was fixed), and single-digit fabrication/catch-rate counts are still too small to treat as statistically stable estimates — they're illustrative of the mechanisms working, not production-grade confidence intervals. A larger hand-labeled eval set remains the highest-leverage next step for making every number in this README more trustworthy.
...
- **A larger calibration set** — the 25/24 calibration/test split (up from 14/14) is a meaningful improvement, but the current coverage figure (83.3%, 15/18 high-confidence test cases) should still be treated as directionally consistent with the 80% target rather than a statistically precise measurement; a larger hand-labeled eval set would tighten this further
