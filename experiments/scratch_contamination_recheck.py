...
QUERIES = [
    "What is the typical length of a DocuSign subscription contract?",
    "What competitive factors does Atlassian identify against its competitors?",
    "What risks does Zoom identify related to devices and networks or emerging technologies?",
]
...
for query in QUERIES:
    ...
    print("=" * 90)
    print(f"Query: {query}")
    print(f"Detected company filter: {filter_source}\n")

    wrong_count = 0
    for rank, r in enumerate(results, start=1):
        is_wrong = filter_source is not None and r["source_file"] != filter_source
        if is_wrong:
            wrong_count += 1
